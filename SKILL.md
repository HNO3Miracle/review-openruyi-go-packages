...
声明间距遵循仓库和 go2spec 的固定模板：普通字段的值通常从第 17 列开始，宏定义通常从第 25 列开始；字段前缀过长时至少保留两个空格。`#!RemoteAsset:` 使用仓库 hook 要求的固定双空格语法。具体边界见格式检查参考。
...
- SPEC 中一眼看不出用途、原因或作用范围的内容必须写注释，尤其手写阶段、模块遍历、目录搬运、依赖过滤与测试特殊处理；注释解释为什么这样做，而不是逐行复述命令。
- 任何时候都不能删除原作者信息。Backport、发行版或其他来源的 patch 均保留原作者与来源记录；具体核验见格式检查。
- 在打包或调整包边界的任务中，能打完整大包就必须打大包，即使引入很多依赖；补齐依赖并保留完整测试范围。具体边界检查见语义检查。
- 历史重写不能覆盖已有修复；开始工作与推送前核对真实 PR head，推送后同步实际工作分支。完整操作见格式检查的历史保护规则。
- pre-commit、补丁应用、源码测试、RPM 元数据和 OBS 构建是不同证据，不能互相代替或提前宣称成功。
